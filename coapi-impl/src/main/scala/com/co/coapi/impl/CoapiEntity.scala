...
/**
  * This is an event sourced entity. It has a state, [[CoapiState]], which
  ...
  */
class CoapiEntity extends PersistentEntity {

  override type Command = CoapiCommand[_]
  override type Event = CoapiEvent
  override type State = CoapiState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: CoapiState = CoapiState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case CoapiState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        CoapiState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class CoapiState(message: String, timestamp: String)
...
object CoapiState {
  /**
    ...
    */
  implicit val format: Format[CoapiState] = Json.format
}

/**
  * This interface defines all the events that the CoapiEntity supports.
  */
sealed trait CoapiEvent extends AggregateEvent[CoapiEvent] {
  def aggregateTag = CoapiEvent.Tag
}

object CoapiEvent {
  val Tag = AggregateEventTag[CoapiEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends CoapiEvent
...
/**
  ...
  */
sealed trait CoapiCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends CoapiCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends CoapiCommand[String]
...
/**
  ...
  * application loader.
  */
object CoapiSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[CoapiState]
  )
}
